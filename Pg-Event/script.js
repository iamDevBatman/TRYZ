const evento = {
  ...
  contato: {
    ...
    redes: [
      { nome: "twitter/alok", icone: "fab fa-x-twitter", url: "https://twitter.com/alok" },
      { nome: "instagram/alok", icone: "fab fa-instagram", url: "https://instagram.com/alok" },
      { nome: "facebook/alok", icone: "fab fa-facebook", url: "https://facebook.com/alok" }
    ]
  },
...
function renderMain() {
  ...
  const descricao = `
    <div class="card box mb-0 p-3">
      ...
      ${evento.descricao.map(p => `<p class="text-white-50">${p}</p>`).join("")}
    </div>
  `;
...
`;
    ...
    <div class="card box mb-0 p-3 text-white">
      ...
      <div class="row">
        <div class="col-md-6">
          ...
          ${evento.contato.redes.slice(0, 2).map(r => `<p><i class="${r.icone} mr-2"></i>${r.nome}</p>`).join("")}
        </div>
        <div class="col-md-6">
          <p><i class="fas fa-envelope mr-2"></i>${evento.contato.email}</p>
          <p><i class="fab fa-whatsapp mr-2"></i>${evento.contato.telefone}</p>
          <p><i class="${evento.contato.redes[2].icone} mr-2"></i>${evento.contato.redes[2].nome}</p>
        </div>
      </div>
    </div>
  `;
  ...
  const categorias = `
    <div class="card box mb-0 p-3">
      ${evento.categorias.map(cat => `<button class="btn btn-outline-light btn-sm mr-2">${cat}</button>`).join("")}
    </div>
  `;
  ...
  const local = `
    <div class="card box mb-0 p-3 text-white">
      ...
      <div class="embed-responsive embed-responsive-4by3">
        <iframe class="maps embed-responsive-item" src="${evento.mapa}" allowfullscreen></iframe>
      </div>
    </div>
  `;
